...
It maps the repository's scaffold, registry, intake, and bridge-requirement layers. It does not create a theorem, claim progress on the Hodge conjecture, or assert that any registered artifact supplies algebraic cycles.
...
5. The Heller-Godel intake and artifact registry handle the current external proof-workbench stream.
6. The bridge requirements scaffold defines what would be required before method artifacts could even be evaluated for Hodge-facing promotion.
7. This program map orients readers and future agents across the whole structure.
...
| --- | --- | --- |
| `core-hodge` | README charter, anti-seed, claim-boundary, Hodge-origin registry as boundary/control layer | Program structure only; no theorem claim |
...
| `hodge-fallout` | No active registry entry yet | Empty as explicit fallout lane |
...
5. a Heller-Godel intake;
6. a Heller-Godel artifact registry;
7. a Hodge bridge requirements scaffold;
8. this program map.
...
2. complete README-advertised structural documents;
3. maintain the Hodge Origin Registry as artifacts are added;
4. add the obstruction registry;
5. only then evaluate Heller-Godel artifacts against the Hodge bridge requirements;
6. require explicit constructions before theorem-facing Hodge claims.

## Next missing README-advertised document

The next missing README-advertised document is:

```text
docs/obstruction-registry.md
```

It should record known obstruction classes and downgrade rules in registry form, using `docs/anti-seed-hodge.md` as the source of truth.
